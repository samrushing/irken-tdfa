...
The DFAs generated by this library do not always provide the correct
POSIX leftmost-longest match when faced with ambiguous regexes.  I
would like to fix this.  The simplest example I've found is the regex
``.*{a+b*a+}``, which will ditch information about the longer match
when it sees the second string of ``a`` characters.
